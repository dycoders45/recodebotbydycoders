...
"https://chat.whatsapp.com/JiotZKYEgWb0X5JK6DZEJe"
`+readmore+`
...
if (!isGroup) return onlyGroup()
 let yui = fs.readFileSync('./temp/audio/halo.mp3')
let niu = [
...
if (!isGroup) return onlyGroup()
if (!m.text.includes('.')) return;
let hoshino = [ "https://telegra.ph/file/8b71af37d07670a5f2228.jpg","https://telegra.ph/file/8b71af37d07670a5f2228.jpg" ]
let emot = [ '🔁','🔂','✅' ]
ehz.sendMessage(m.chat, {
    react: {
...
sourceUrl: 'https://whatsapp.com/channel/0029VaBqOJmLdQeUw9URW60kS',
mediaType: 1,
renderLargerThumbnail: true 
}}}, { quoted: m,ephemeralExpiration: 86400});  
await sleep(2000)
ehz.sendMessage(m.chat,{
audio: ngel,mimetype:'audio/mp4', ptt:false })
}
break
...
let ini_gen = `${command}`
let get_result = await getBuffer(`https://skizo.tech/api/toanime?url=${wewek}&apikey=dycoders`)
...
await sleep(1000)
axios.get(`https://api.lolhuman.xyz/api/simi?apikey=${api.Lol}&text=${q}&badword=false`).then(({ data }) => {
...
if (args.length == 0) return m.reply(`Contoh: ${prefix + command} kids,cute`)
		      m.reply(mess.wait)
 ehz.sendMessage(m.chat, { image: { url: `https://skizo.tech/api/txt2img?text=${q}&apikey=dycoders` }, caption: `${botName}\n\n Type: ${command}\n\nText: ${q}`})
 }
			break
 case 'ebinary': {
...
setReply(mess.wait)
let res = await fetchJson(`https://skizo.tech/api/ttsearch?search=${q}&apikey=dycoders`)
...
if (!isGroup) return onlyGroup()
	if (!isPremium && global.db.data.users[sender].limit < 1) return setReply(mess.limit) // respon ketika limit habis
...
break
		...
		case 'spotifysearch':
			...
			axios.get(`https://api.lolhuman.xyz/api/spotifysearch?apikey=${api.Lol}&query=${q}`).then(({ data }) => {
				var text = ''
				for (var x of data.result) {
					text += `Title : ${x.title}\n`
...
let Url = `https://api.apiflash.com/v1/urltoimage?access_key=d0e32ee368d344b68fcd67bbfe331bd5&url=${q}&format=png&full_page=true&no_cookie_banners=true&no_ads=true&wait_until=network_idle`
...
if (!q) return setReply("Masukan Link")
if (!isUrl(args[0]) && !args[0].includes('www.')) return setReply("Link error");
let Tablet = `https://api.apiflash.com/v1/urltoimage?access_key=d0e32ee368d344b68fcd67bbfe331bd5&url=${q}&width=800&height=1280&quality=80&no_cookie_banners=true&no_ads=true&no_tracking=true&wait_until=network_idle`
ehz.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${botName}`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: vid.url,sourceUrl: `https://wa.me/${nomerOwner}`
...
if (!q) return setReply("Masukan Link")
if (!isUrl(args[0]) && !args[0].includes('www.')) return setReply("Link error");
let Ssweb = `https://api.apiflash.com/v1/urltoimage?access_key=d0e32ee368d344b68fcd67bbfe331bd5&url=${q}&format=png&width=480&height=800&no_cookie_banners=true&no_ads=true&wait_until=network_idle`
ehz.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${botName}`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: vid.url,sourceUrl: `https://wa.me/${nomerOwner}`
...
//======== MENU OWNER ======//
 ...
 case 'clearuser':
	...
	case 'jooxplay':
			if (args.length == 0) return setReply(`Example: ${prefix + command} Melukis Senja`)
			axios.get(`https://api.lolhuman.xyz/api/jooxplay?apikey=${api.Lol}&query=${full_args}`).then(({ data }) => {
